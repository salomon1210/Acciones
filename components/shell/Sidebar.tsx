...
export function Sidebar() {
  ...
  const { sidebarCollapsed, toggleSidebar, setCommandOpen } = useUI();
    ...
    >
          ...
          {NAV.map((item) => {
            ...
            const link = (
              <Link
                href={item.href}
                className={cn(
                  "flex h-8 items-center gap-2 rounded px-2 text-xs transition-colors",
                  active
      ...
      <div className="border-t border-border p-2">
        <button
          ...
          className={cn(
            "flex w-full items-center gap-2 rounded border border-border bg-bg px-2 py-1.5 text-xs text-fg-dim hover:text-fg transition-colors",
